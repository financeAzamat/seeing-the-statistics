...
# ci.render.js so BOTH locales can load the same file -- a Russian copy of the
# page would otherwise have carried its own duplicate of 465 lines, and the two
...
if inline:
    ...
    if not ext:
        fails.append("ci.html has neither an inline script nor a *.render.js")
...
if r.returncode:
    fails.append("the ci renderer does not parse")
