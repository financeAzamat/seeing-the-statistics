...
window.UDJ_STRINGS = {
  ...
  /* ---- Раздел 9 · точность и матрица ошибок ---------------------------
     Терминология. «Точность» закреплена за accuracy — так названа запись
     раздела в содержании. Поэтому precision и recall переданы простыми
     словами: «меткость» — про попадания, «полнота» — про охват. Английские
     термины даны в разделе «На языке экзамена», для которого он и нужен. */
  '__unit_ct': 'кар',

  'Is this diamond worth more than $10,000?': 'Этот алмаз дороже $10 000?',
  ['Every diamond in the ggplot2 set, labelled by whether its price is above '
   + '$10,000, predicted from the one thing you can measure without a grader: '
   + 'its size.']:
    'Каждый алмаз из набора ggplot2, помеченный по тому, выше ли его цена '
    + '$10 000, и предсказанный по единственному, что можно измерить без '
    + 'оценщика: по размеру.',
  'all 53,940 records used for every rate below':
    'все 53 940 записей использованы для каждой доли ниже',
  /* Источник и строка про выборку уже переведены выше — этот раздел берёт
     готовые записи, чтобы «seed» не превратился в «сид» на одной странице. */

  'truth line at {0} · decision line at {1} · drag it':
    'линия истины: {0} · линия решения: {1} · тяните её',
  'THE TRUTH': 'ИСТИНА',
  '{0} shown': '{0} на экране',

  'TRUE POSITIVE': 'ВЕРНО ПОМЕЧЕН',
  'FALSE POSITIVE': 'ЛОЖНО ПОМЕЧЕН',
  'FALSE NEGATIVE': 'ЛОЖНО ПРОПУЩЕН',
  'TRUE NEGATIVE': 'ВЕРНО ПРОПУЩЕН',
  'flagged, and it was': 'помечен, и верно',
  'flagged, but cheap': 'помечен, но дешёвый',
  /* Ключи в кавычках: без них извлекатель ключей не видит запись в таблице
     и считает её осиротевшей, хотя код читает её через TR(q.s) и TR(r.n). */
  'missed': 'пропущен',
  'correctly ignored': 'верно оставлен',

  /* Плашка вердикта стоит в одной строке с заголовком в половинной панели,
     поэтому и заголовок, и плашка короткие: длинная плашка наезжала на
     последнее слово заголовка — на обеих локалях. */
  'the score everyone quotes': 'самое цитируемое число',
  'REAL GAIN': 'ЕСТЬ ВЫИГРЫШ',
  'NO GAIN': 'БЕЗ ВЫИГРЫША',
  'accuracy {0} · doing nothing scores {1}':
    'точность {0} · бездействие даёт {1}',
  'DOING NOTHING': 'БЕЗДЕЙСТВИЕ',
  'scale starts at {0} — both numbers live up here':
    'шкала начинается с {0} — оба числа живут здесь',
  '{0} points better than always predicting "no"':
    '{0} пункта лучше, чем всегда отвечать «нет»',
  '{0} points worse than always predicting "no"':
    '{0} пункта хуже, чем всегда отвечать «нет»',

  'the two that cannot be gamed': 'две меры, которые не подделать',
  'precision {0} · recall {1} · F1 {2}':
    'меткость {0} · полнота {1} · F1 {2}',
  'PRECISION': 'МЕТКОСТЬ',
  'RECALL': 'ПОЛНОТА',
  'drag the line: precision and recall trade against each other':
    'тяните линию: меткость и полнота тянут в разные стороны',

  'at {0}': 'на {0}',
  '{0} points of real gain.': '{0} пункта настоящего выигрыша.',
  'no real gain at all.': 'настоящего выигрыша нет вовсе.',
  ['At {0} you flag {1} stones, {2} of them correctly, and miss {3} '
   + 'expensive ones.']:
    'На {0} вы помечаете {1} камней, из них {2} верно, и упускаете {3} дорогих.',
  'Accuracy {0} against {1} for predicting "no" every time — {2}':
    'Точность {0} против {1}, если всегда отвечать «нет» — {2}',
  ['Every rate is measured over all {0} records across {1} candidate cut-offs, '
   ...
   + 'dots are visible in it.']:
    'Каждая доля измерена по всем {0} записям на {1} возможных порогах, поэтому '
    + 'ничто на экране не интерполировано. На диаграмме показаны {2} случайные '
    + 'точки, и в каждом квадранте напечатаны и счёт по всему набору, и число '
    + 'видимых точек.',
  'Cut-off pushed to {0}: accuracy {1}, recall {2}.':
    'Порог сдвинут на {0}: точность {1}, полнота {2}.',
  'Best accuracy at {0}: accuracy {1}, recall {2}.':
    'Лучшая точность на {0}: точность {1}, полнота {2}.',
  'Best F1 at {0}: accuracy {1}, recall {2}.':
    'Лучший F1 на {0}: точность {1}, полнота {2}.',
};
